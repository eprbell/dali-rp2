...
# DaLI for RP2 v0.6.0 Developer Guide
...
* implement the `_process_gains()` method, which reads data for gains using [the implicit API of CCXT](https://docs.ccxt.com/en/latest/manual.html#implicit-api) received and accepts a list of [InTransaction](src/dali/in_transaction.py) instances, and [OutTransaction](src/dali/out_transaction.py) instances. If the exchange doesn't support gains, the plugin can use `pass` in the method.
