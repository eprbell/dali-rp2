...
* **[Data Loader Plugin Sections](#data-loader-plugin-sections)**
  ...
  * [Binance.com Section (CSV)](#binance.com-section-csv)
...
This plugin is REST-based and requires setting up API Keys in your Binance.com account settings (click on the API link).
...
* Due to the information not being available via REST, autoinvest trades and ETH to BETH conversions can not be processed with this plugin. Please download the CSV for these transactions and use the Binance.com CSV plugin. 
...
### Binance.com Section (CSV)
This plugin is CSV-based and parses CSV files generated by Binance.com. It currently only supports autoinvest purchases and ETH to BETH conversions. Initialize it as follows:
<pre>
...
* `fiat_priority` is an optional list of strings in JSON format that ranks the priority of fiat in the routing system. If no `fiat_priority` is given, the default priority is `["USD", "JPY", "KRW", "EUR", "GBP", "AUD"]`, which is based on the volume of the fiat market paired with BTC (ie. BTC/USD has the highest worldwide volume, than BTC/JPY, etc.).
...
For example, if a user needs the price of their BETH (Beaconed ETH) purchase on Binance.com in CHF (Swiss Francs). The router will first route the price through the available markets on the exchange:
