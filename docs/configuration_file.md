...
* **[Data Loader Plugin Sections](#data-loader-plugin-sections)**
  ...
  * [Crypto.com App Section (CSV)](#cryptocom-app-section-csv)
...
### Crypto.com App Section (CSV)
This plugin is CSV-based and parses CSV files generated by the Crypto.com App (Not Exchange). It currently supports the following transition "kinds":

| n  | Kind                              | Mapped to:                                  |
|----|-----------------------------------|---------------------------------------------|
| 1  | referral_gift                     | INCOME (Signup bonus)                       |
| 2  | viban_purchase                    | BUY (Buying crypto with fiat)               |
| 3  | reimbursement                     | INCOME (Cashback from promos)               |
| 4  | referral_card_cashback            | INCOME (Cashback from debit card use)       |
| 5  | mco_stake_reward                  | STAKING                                     |
| 6  | crypto_earn_interest_paid         | INTEREST (Interest from earn program)       |
| 7  | crypto_purchase                   | BUY (Buy crypto with fiat)                  |
| 8  | crypto_viban_exchange             | SELL (Sell crypto to fiat)                  |
| 9  | card_top_up                       | SELL (Funding debit card)                   |
| 10 | reimbursement_reverted            | SELL or remove original (Reward Reverted)   |
| 11 | card_cashback_reverted            | SELL or remove original (Cashback Reversal) |
| 12 | crypto_withdrawal                 | MOVE (Not correctly mapped at the moment)   |
| 13 | crypto_exchange                   | SELL and BUY (Exchange between cryptos)     |
| 14 | viban_deposit_precredit           | IGNORE (Credit right after bank deposit)    |
| 15 | viban_deposit_precredit_repayment | IGNORE (Bank deposit cleared)               |
| 16 | lockup_lock                       | IGNORE (Lock for debit card)                |
| 17 | lockup_unlock                     | IGNORE (Unlock from Debit Card)             |
| 18 | crypto_earn_program_created       | IGNORE (Lock for earn program)              |
| 19 | crypto_earn_program_withdrawn     | IGNORE (Unlock from earn program)           |

<pre>
[dali.plugin.input.csv.crypto_com_app <em>&lt;qualifiers&gt;</em>]
account_holder = <em>&lt;account_holder&gt;</em>
in_csv_file = <em>&lt;in_csv_file&gt;</em>
remove_reverted_transactions = <em>&lt;boolean&gt;</em>
</pre>

IMPORTANT NOTES:
* This plugin has been tested with an export done on March-2025 with USD as native currency. It may need work for different configurations.
* I did my best to map the transaction kinds to the correct transaction types, but there may be some mistakes. Please review them carefully.
