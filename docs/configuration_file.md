...
<!-- There are issues accessing Chinese websites from certain foreign jurisdictions, so disabling our link checking on Binance links -->
...
end_date = <em>&lt;end_date&gt;</em>
</pre>

Note: the `thread_count`, `use_cache`, and `end_date` parameters are optional. The `end_date` parameter accepts an ISO date string (e.g., "2025-12-31") to filter transactions, limiting the fetch to transactions on or before the specified date. This is useful for limiting data to a specific tax year (e.g., "2025-12-31" for tax year 2025).
