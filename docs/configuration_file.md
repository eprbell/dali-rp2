...
Where:
* *`price`* is one of `open`, `high`, `low`, `close`, `nearest`, `weighted`. When DaLi downloads historical market data, it captures a `bar` of data surrounding the timestamp of the transaction. Each bar has a starting timestamp, an ending timestamp, and OHLC prices. You can choose which price to select for price lookups. The open, high, low, and close prices are self-explanatory. The `nearest` price is either the open price or the close price of the bar depending on whether the transaction time is nearer the bar starting time or the bar ending time.  The `weighted` price takes this idea a step further and calculates a weighted average of the open and close prices, where the weights of the open and close prices are a function of the transaction time relative to the bar starting time and ending time. When the transaction time is near the bar starting time, the weighted price will be near the open price. As the transaction time approaches the bar ending time, the weight of the open price decreases and the weight of the close price increases. When the transaction time is near the bar ending time, the weighted price will be near the close price. Note: be aware that unlike other price selection choices, the weighted price formula will generate a price with many extra digits of precision after the decimal point.
